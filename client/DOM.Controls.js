  ...
  Controls.Modification = Common.jQuery.extend('HeroWO.DOM.Controls.Modification', {
    ...
    events: {
      ...
      attach: function () {
        ...
        this.autoOff(this.sc.transitions, {
          nestExNew: function (res) {
            this._debouncedUpdate()
            // Using _opt to not trip the logging hook on transition change that assumes only simple objects are set to its options (as is normally the case).
            res.child.get('transition')._opt._view = res.child
      ...
      _update: function () {
        ...
        var tr = _.invoke(this.map.transitions.nested(), 'get')
    ...
    elEvents: {
      ...
      'click .eff-add': function () {
        ...
        _.each(code.split('}'), function (str) {
          try {
