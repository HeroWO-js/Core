define(['DOM.Common', 'Calculator', 'ObjectStore'], function (Common, Calculator, ObjectStore) {
  ...
  // This module and Browser.js provide the following features to aid in debugging:
  ...
  //* the `'debug variable in Browser.js, to enable production, debug or tracing mode
    ...
    events: {
      ...
      attach: function () {
        ...
        this.cx.mixIn({
          ...
          // When `'debug in `[Browser.js`] is 2+, dumps active Module-s and stack traces of take()/release() calls (for TakeRelease objects). Use this to diff engine state before and after some action to validate proper clean-up of unused objects.
          trace: function (obj) {
            if (!obj) {
              var res = []
              var alive = []
