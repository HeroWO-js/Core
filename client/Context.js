define(['Common', 'Map', 'Calculator', 'require'], function (Common, HMap, Calculator, require) {
  ...
  var IdleTasks = Common.Sqimitive.extend('HeroWO.Context.IdleTasks', {
    ...
    queue: function (func, cx) {
      ...
      //
      // XXX to avoid lags in single-player mode we might try implementing master-slave based on Web Workers (master as a Worker, main page as a slave to it, communicating by postMessage() rather than WS)
      if (this._tasks) {
  ...
  // are added by the client (such as `[Browser.js`] adding `#Screen) when there is a transition from the empty
  ...
  //   cx.remove()
  // `]
  return Common.Sqimitive.extend('HeroWO.Context', {
    ...
    //  powers like ability to kick players.
    //
    ...
    //  `> cause string `- optional loading initiator; currently, `'rpc is given by `@RPC.WebSocket.Connector`@
    //  `> configure bool
    //  `> master bool
    //
