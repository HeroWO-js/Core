    ...
    _hookWorld: function () {
      ...
      function hookCombat(combat, locator) {
        ...
        hooks.push([combat.queue, combat.queue.on('^-1^nestExNew', function (res) {
          self.serverEvent('combat_queue', {phase: self._contextPhase(), locator: locator, args: [_.pick(res, 'key', 'pos', _.forceObject), res.child._parentKey]})
        })])
      ...
      function hookStore(store, locator, superStore) {
        ...
        locator.updateLocator = function () {
          if (superStore) {
            var size = store.size()
