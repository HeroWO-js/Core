          ...
          case MATCH[1]:
            ...
            if (this._opt.listen >= 2) {
              if (!indexes.manyListeners) {
                // More correct in theory would be to set up Effect change hooks when matching each selector (in _test()) but this will result in a large number of listeners (see the comment in AFFECTORS_IN), thus we re-test the Effect if any of its property changes which is fast enough.
