  ...
  Bits.Windows = Bits.Base.extend('HeroWO.DOM.Bits.Windows', {
    ...
    events: {
      ...
      '-nestEx': function (options) {
        // withinWindow restricts new child's top to be less than the next fullScreen Window after withinWindow (if any). It's meant for cases when there are several screens (fullScreen windows, like ADVMAP and combat(s)) and an overlaid window may produce windows that should not overlay the currently visible screen. It does that by specifying itself in their withinWindow. This allows user to see background messages upon leaving the topmost screen.
        options.pos = ++this._z
