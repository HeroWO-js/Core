define([
  ...
  var Rules = Common.Sqimitive.extend('HeroWO.H3.Rules', {
    ...
    _triggerObjectEffects: function (id, actor, toPending, moveTransition, unique) {
      ...
      // XXX+I: mof: implement monster fleeing (using retreatCan)
          ...
          this.map.shroud.batch(null, function () {
            state.forEach(function (cur) {
              if (cur[0] == 2) {
                ...
                // There was a change to "explored" (or "invisible"). It doesn't imply the cur spot isn't "fully visible" (or "explored") in some ally (the player's visibility state may have lowered but another player may still oversee the spot) so gotta check that by clearing allies... bits, testing and possibly setting one of them again. Doing all this flipping within one batch doesn't incur side effects like mini-map render because Shroud coalesces the changes and if we restore a bit to its original state then the change is eliminated entirely.
                players.forEach(function (player) {
                  // The change's player has never any of allies... bits set since he "owns" the spot.
                  if (player != cur[4]) {
                    // If the bit was unset by this call, we know the other bit is already unset since we have only one of them set at a time.
                    //
                    // However, we don't know which of these bits is set; old of 'changes' could tell that but it's missing if the change is coming from Effects.
                    if (this.map.shroud.setAtCoords(cur[1], cur[2], cur[3], player, alliesExplored, false) == null) {
                      this.map.shroud.setAtCoords(cur[1], cur[2], cur[3], player, alliesVisible, false)
                    }
                  }
                }, this)

                var player = cur[0] == 1 ? cur[4] : null
                var bit = alliesExplored
                _.some(players, function (p) {
                  if (p != cur[4]) {  // we already know the state of cur[4]
                    var vis = this.map.shroud.atCoords(cur[1], cur[2], cur[3], p)
                    if (vis >= 0) {
                      player = p
                      if (visible.indexOf(vis) != -1) {
                        bit = alliesVisible
                        return true
                      }
                    }
                ...
                if (player != null) {
                  players.forEach(function (p) {
                    if (player != p) {
                      this.map.shroud.setAtCoords(cur[1], cur[2], cur[3], p, bit, true)
                    }
                  }, this)
                }
              }
