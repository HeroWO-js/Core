                  ...
                  .on({
                    submit: function () {
                      if (this.get('hire') > 0) {
                        ...
                        if (slot >= 7) {
                          this.sc.modules.nested('HeroWO.H3.DOM.UI').windows.addModule(H3Bits.MessageBox, {withinWindow: win})
                            // GENRLTXT.TXT[426]
      ...
      attach: function () {
        ...
        this.autoOff(this.sc, {
          ...
          cellRightClick: function (x, y, z) {
            ...
            if (this.sc.get('mapShroud') && !(this.map.shroud.atCoords(x, y, z, this.pl.get('player')) >= 0)) {
              return this.sc.modules.nested('HeroWO.H3.DOM.UI').windows.addModule(H3Bits.MessageBox, {withinWindow: this, elClass: 'Hh3-msg_small', centerIn: cell, tooltip: true})
            ...
            if (objects.length) {
              switch (_.last(objects).type) {
                case this.map.constants.object.type.monster:
                  ...
                  var msg = this.sc.modules.nested('HeroWO.H3.DOM.UI').windows.addModule(H3Bits.MessageBox, {withinWindow: this, elClass: 'Hh3-msg_small', centerIn: cell, tooltip: true})
        ...
        heroList.on({
          '.clicked': function (item) {
            if (item.get('selected')) {
              this.get('ui').windows.addModule(UI.HeroInfo, {withinWindow: this, hero: item.get('object')})
            }
          },
        }, this)
        ...
        this.fuse('submit', function () {
          ...
          if (cur = heroList.currentObject()) {
            this.get('ui').windows.addModule(UI.HeroInfo, {withinWindow: this, hero: cur})
        ...
        this.addModule('options', H3Bits.Button, {attachPath: buttons, elClass: 'Hh3-btn_id_IAM009 Hsfx__btn'})
          .on('clicked', function () { this.get('ui').windows.addModule(UI.GameOptions, {withinWindow: this}) }, this)
            ...
            if (hero && hero.isHero) {
              ...
              if (l != null && this.map.bySpot.atCoords(x, y, z, 'actionable', l) === this.map.constants.spotObject.actionable.actionable) {
                ...
                if (this.cx.get('classic') || !l) {
                  this.sc.modules.nested('HeroWO.H3.DOM.UI').windows.addModule(UI.HeroInfo, {withinWindow: this.sc.modules.nested('HeroWO.H3.DOM.UI').windows.nested('map'), hero: hero})
                } else {
                        ...
                        case this.map.constants.object.type.hero:
                          ...
                          if (this.map.players.nested(owner).get('team') == this.pl.get('team')) {
                            if (owner == this.pl.get('player')) {
                              ...
                              this.sc.modules.nested('HeroWO.H3.DOM.UI').windows.addModule(UI.HeroInfo, {withinWindow: this.sc.modules.nested('HeroWO.H3.DOM.UI').windows.nested('map'), hero: this.map.representationOf(id)})
                            }
                          }
                          return town = false
                        case this.map.constants.object.type.town:
              ...
              submit: function () {
                if (!win._buyButton.get('disabled')) {
                  ...
                  win.autoOff(async, {})
                    .whenSuccess(function () {
                      win.cancel()
                      win.sc.modules.nested('HeroWO.H3.DOM.UI').windows.addModule(H3Bits.MessageBox, {withinWindow: this})
    ...
    _attachGarrison: function () {
      ...
      if (this.get('dismissButton') == null) {
        ...
        this.on('dismiss', function () {
          var box = this.sc.modules.nested('HeroWO.H3.DOM.UI').windows.addModule(H3Bits.MessageBox, {withinWindow: this})
        ...
        this.on('upgrade', function () {
          ...
          switch (to.length) {
            case 0:
              return this.sc.modules.nested('HeroWO.H3.DOM.UI').windows.addModule(H3Bits.MessageBox, {withinWindow: this})
  ...
  UI.GameOptions = H3Combat.BaseOptions.extend('HeroWO.H3.DOM.UI.GameOptions', {
    ...
    events: {
      attach: function () {
        ...
        // Disallow in multi-player because different classic setting on master/client may cause discrepancies. Also disallow in hotseat because hotseat is not compatible with simultaneous turns (see Browser.js).
    ...
    events: {
      ...
      showTooltip: function () {
        ...
        details && this.sc.modules.nested('HeroWO.H3.DOM.UI').windows.addModule(UI.Bits.RightPanel.Hero, {
          withinWindow: this.sc.modules.nested('HeroWO.H3.DOM.UI').windows.nested('map'),
          elClass: 'Hh3-rp_tooltip',
          hero: this.get('object'),
    ...
    events: {
      owned: function () {
        ...
        this.addModule('garrison', H3Bits.GarrisonList, {
          sink: {'*': {elClass: 'Hh3-am-rp__garrison-cr', options: {details: this.get('details')}}},
