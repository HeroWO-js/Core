define(['RPC.Common', 'RPC'], function (Common, RPC) {
  ...
  // Internal class for `@RPC.WebSocket`@ holding requests issued to the remote side having no responses received so far.
  var Pending = Common.Sqimitive.extend({
  ...
  var WS = RPC.Client.extend('HeroWO.RPC.WebSocket', {
    ...
    events: {
      ...
      // called immediately after send(). Normally you'd wait until responses
      ...
      '=do': function (sup, method, args) {
        ...
        _.log && _.log('<[%s]= %s %j', async._parentKey, method, args)

        async.on('change_status', function (now) {
          _.log && _.log('=[%s]> %s : %s %s',
                         async._parentKey, method, now ? 'SUCCESS' : 'ERROR',
                         async.errorResult ? async.errorResult.message : '')
        })

        // Unlike with inherited do(), async's status may only change in response
        // to a WebSocket message which cannot happen during this tick, thus no
        // need for defer() - the caller intending on handling errors must call
        // whenError() as do() returns, during this tick.
        async.whenError(function () {
          async.get('errorHandlers') > 1 || async.rethrow()
        }, null, Infinity)

        // This relies on the fact send() can't throw, else the caller won't have
    ...
    handleJsonRpcMessage: function (data) {
      ...
      if (!async) {
        ...
        console && console.warn('JSON-RPC Response to unknown Request: ' + data.id)
      } else if (!data.error) {
        ...
        async.set('status', false)
      } else {    // generic error without a reference to any Request
        console && console.warn('JSON-RPC error: ' + data.error.message)
      }
    },
          ...
          .whenSuccess(function (player) {
            ...
            newAsync(this.do('start'))
              .whenSuccess(function (start) {
                ...
                cx.game({url: start.result.url, data: start.result, configure: start.result.configure, cause: 'rpc'})
              })
          }, this)
      }
      return async
