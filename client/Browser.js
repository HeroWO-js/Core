    ...
    $(function () {
      ...
      if (debug) {
        ...
        function updatePrompt(now) {
          ...
          var prompt = cx.get('screen') == 'game' && (!cx.get('loading') || (!cx.get('master') && (!menu || !menu.get('options') || menu.get('options').get('host'))))
      ...
      updaters.nested('maps').on('_replaceAll', function () {
        if (!debug && menu.get('screen') == 'newSingle' && !window.herowoAiMessage) {
      ...
      cx.on('-menu', function (sc, options) {
        ...
        switch (options) {
          case 'save':
            options = {
              screen: options + (cx.get('master') ? 'Single' : 'Multi'),
              section: 'list',
            }
            break
          case 'load':
      ...
      function initChat() {
        ...
        if (cx.get('screen') == 'game' && !cx.get('loading')) {
          ...
          if (cx.get('screen') == 'game' && !cx.get('master') && chatWS &&
    ...
    function start(multi, data) {
      ...
      //      .append(async.response)
          ...
          if (multi) {
            ...
            hooks.push([cx, cx.on('-dataReady', function () {
              if (!cx.players.some(Common.p('isHuman'))) {
                cx.players.some(function (pl) {
