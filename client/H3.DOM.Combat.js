define(['DOM.Common', 'PathAnimator', 'Calculator', 'Effects', 'Map', 'DOM.Slider', 'DOM.Bits', 'H3.DOM.Bits', 'H3.Combat', 'H3.Rules'], function (Common, PathAnimator, Calculator, Effects, HMap, Slider, Bits, H3Bits, H3Combat, Rules) {
      ...
      hit_tick: function (view, async) {
        switch (view.get('type')) {
          ...
          case 'combatHit':
            ...
            if (this.rules.creatures.atCoords(cr.get('creature'), 0, 0, 'damageGroup', 0) == this.rules.constants.creature.damageGroup.wall) {
              return
            }
    ...
    events: {
      attach: function () {
        ...
        this._calcs.creature_strikes = this.cx.listeningEffectCalculator({
          update: 'defer',
          target: this.map.constants.effect.target.creature_strikes,
          shared: false,
          ifCombat: this.combat._parentKey,
          ifCombatCreature: this.state.get('creature')._parentKey,
        })
        ...
        this._calcs.creature_wallStrikes = this.cx.listeningEffectCalculator({
          update: 'defer',
          target: this.map.constants.effect.target.creature_wallStrikes,
          shared: false,
          ifCombat: this.combat._parentKey,
          ifCombatCreature: this.state.get('creature')._parentKey,
        })
  ...
  Combat.Map.Mode.FirstAid = Combat.Map.Mode.extend('HeroWO.H3.DOM.Combat.Map.Mode.FirstAid', {
    ...
    _updateOwnSpot: function () {
      ...
      this.map.walkImpassable(this.state.get('creature'), function (o) {
        this.cmap.gridCellAt(o.mx, o.my).classList.add(clsS)
      }, this)
    },
  })
    ...
    events: {
      attach: function () {
        ...
        this._calcToOpt('global', Calculator.Effect.GenericBool, {
          target: this.map.constants.effect.target.spellGlobal,
        })

        this._potential = new Effects.Collection({effects: this.map.effects})
        ...
        this._potential.fuse('+readyMember', function (res, creature) {
          ...
          res.calc = this.cx.listeningEffectCalculator({
            class: Calculator.Effect.GenericBool,
            update: 'defer',
            target: this.map.constants.effect.target[this._immunityTarget],
            shared: false,
            ifCombat: this.combat._parentKey,
        ...
        if (cr) {
          ...
          if (calc.get('value')) {
            ...
            calc.get('affectors').concat().reverse().some(function (n) {
              ...
              switch (src) {
                case this.map.constants.effect.source.spellDefense:
                case this.map.constants.effect.source.spellOffense:
                case this.map.constants.effect.source.spellImmune:
                  found = src
                  return true
                default:
                  if (src[0] == this.map.constants.effect.source.spell) {
                    return found = src
                  }
              }
            }, this)
    ...
    events: {
      '+_filterMouseCreatures': function (now) {
        return now.filter(function (cr) {
          ...
          var ifCombat = this.map.effects.propertyIndex('ifCombat')
          var ifCombatCreature = this.map.effects.propertyIndex('ifCombatCreature')
          var ifTargetCombatCreature = this.map.effects.propertyIndex('ifTargetCombatCreature')
          var found = this.map.effects.find(source, function (src, $1, $2, $3, $4, n) {
            if (src && src[0] == this.map.constants.effect.source.spell && this.map.effects.atContiguous(n - source + ifCombat, 0) === this.combat._parentKey && (this.map.effects.atContiguous(n - source + ifCombatCreature, 0) === cr._parentKey || this.map.effects.atContiguous(n - source + ifTargetCombatCreature, 0) === cr._parentKey)) {
              return true
            }
          }, this)
          return found
      ...
      attach: function () {
        ...
        _.each(['myObjects', 'enemyObjects'], function (who) {
          ...
          if (!first) {
            ...
            my.addModule('n', H3Bits.DatabankProperty, {
              el: false,
              collection: 'creatures',
          ...
          if (!first) {
            ...
            this.addModule(who + 'Face', H3Bits.CreatureImage, {
              creature: cr.get('creature'),
              type: 'large',
            })
