...
return [
      ...
      [
        ...
        'wk_$' => [
          ...
          ['quest_message', [$custom, 'rules', 4,
           sprintf(toMarkup($adve[172], '`{Audio GAZEBO`}'), '`SKILL'),
           sprintf(toMarkup($adve[173], '`{Audio GAZEBO`}'), '`SKILL')]],
          ['bonus_message', [$const,
           [sprintf(toMarkup($adve[171], '`<`{SkillImage `SKILLID`, basic`} Basic `SKILL`>`{Audio GAZEBO`}'), '`SKILL')]]],
        ],
      ]
    ),
  ...
  //
  // Objects that have no loop sound on adventure map:
  // altarOfSacrifice, boat, borderGate, borderGuard, cartographer, cloverField,
  // corpse, coverOfDarkness, derelictShip, evilFog, eyeOfMagi, favorableWinds,
  // fieryFields, flotsam, freelancerGuild, frozenLake, holyGround,
  // hutOfMagi, idolOfFortune, keymasterTent, leanTo, libraryOfEnlightenment,
  // lighthouse, lucidPools, magicClouds, magicWell, mermaids, monster, oasis,
  // obelisk, oceanBottle, pandoraBox, prison, pyramid, questGuard,
  // redwoodObservatory, resource, rocklands, scholar, seaChest, seerHut,
  // shipwreck, shipwreckSurvivor, sign, spellScroll, swanPond, treasureChest,
  // treeOfKnowledge, university, wagon, warriorTomb, wateringHole, witchHut.
  'objectOverrides' => $e([
    array_fill_keys($h3Classes[217], [
    ...
    array_fill_keys($c_mine_0, [
      'name' => $mine[0][0],
      'idName' => $mine[0][1],
      'sounds' => [new ClassSound(['sound' => 'LOOPLUMB'])],
    ]),
    // Alchemist's Lab
    array_fill_keys($c_mine_1, [
      'name' => $mine[1][0],
      'idName' => $mine[1][1],
      'sounds' => [new ClassSound(['sound' => 'LOOPSTAR'])],
    ]),
    // Ore Pit
    array_fill_keys($c_mine_2, [
      'name' => $mine[2][0],
      'idName' => $mine[2][1],
      'sounds' => [new ClassSound(['sound' => 'LOOPSULF'])],
    ]),
    // Sulfur Dune
    ...
    array_fill_keys($c_mine_4, [
      ...
      'idName' => 'crystalMine',
      'sounds' => [new ClassSound(['sound' => 'LOOPCRYS'])],
    ]),
    // Gem Pond
    array_fill_keys($c_mine_5, [
      'name' => $mine[5][0],
      'idName' => $mine[5][1],
      'sounds' => [new ClassSound(['sound' => 'LOOPGEMP'])],
    ]),
    // Gold Mine
    array_fill_keys($c_mine_6, [
      'name' => $mine[6][0],
      'idName' => $mine[6][1],
      'sounds' => [new ClassSound(['sound' => 'LOOPMINE'])],
    ]),
    // Abandoned Mine
    array_fill_keys($c_mine_7, [
      'name' => $mine[7][0],
      'idName' => $mine[7][1],
      'sounds' => [new ClassSound(['sound' => 'LOOPCAVE'])],
    ]),
    // Garrison
    array_fill_keys($c_garrison_0, [
      'name' => $garr[0][0],
      'idName' => $garr[0][1],
      'sounds' => [new ClassSound(['sound' => 'LOOPSWAR'])],
    ]),
    // Anti-magic Garrison
    array_fill_keys($c_garrison_1, [
      'name' => $garr[1][0],
      'idName' => $garr[1][1],
      'sounds' => [new ClassSound(['sound' => 'LOOPMAGI'])],
    ]),
    array_fill_keys($c_altarOfAir, ['sounds' => [new ClassSound(['sound' => 'LOOPAIR'])]]),
    array_fill_keys($c_arena, ['sounds' => [new ClassSound(['sound' => 'LOOPAREN'])]]),
    array_fill_keys($c_marlettoTower, ['sounds' => [new ClassSound(['sound' => 'LOOPAREN'])]]),
    array_fill_keys($c_campfire, ['sounds' => [new ClassSound(['sound' => 'LOOPCAMP'])]]),
    array_fill_keys($c_denOfThieves, ['sounds' => [new ClassSound(['sound' => 'LOOPDEN'])]]),
    // Cyclops Stockpile
    array_fill_keys($c_creatureBank_0, ['sounds' => [new ClassSound(['sound' => 'LOOPCAVE'])]]),
    // Dwarven Treasury
    array_fill_keys($c_creatureBank_1, ['sounds' => [new ClassSound(['sound' => 'LOOPDWAR'])]]),
    // Griffin Conservatory
    array_fill_keys($c_creatureBank_2, ['sounds' => [new ClassSound(['sound' => 'LOOPGRIF'])]]),
    // Imp Cache
    array_fill_keys($c_creatureBank_3, ['sounds' => [new ClassSound(['sound' => 'LOOPFIRE'])]]),
    // Medusa Stores
    array_fill_keys($c_creatureBank_4, ['sounds' => [new ClassSound(['sound' => 'LOOPMEDU'])]]),
    // Naga Bank
    array_fill_keys($c_creatureBank_5, ['sounds' => [new ClassSound(['sound' => 'LOOPNAGA'])]]),
    // Dragon Fly Hive
    array_fill_keys($c_creatureBank_6, ['sounds' => [new ClassSound(['sound' => 'LOOPLEAR'])]]),
    array_fill_keys($c_warMachineFactory, ['sounds' => [new ClassSound(['sound' => 'LOOPFACT'])]]),
    array_fill_keys($c_faerieRing, ['sounds' => [new ClassSound(['sound' => 'LOOPFAER'])]]),
    array_fill_keys($c_magicSpring, ['sounds' => [new ClassSound(['sound' => 'LOOPFOUN'])]]),
    array_fill_keys($c_altarOfFire, ['sounds' => [new ClassSound(['sound' => 'LOOPFIRE'])]]),
    array_fill_keys($c_rallyFlag, ['sounds' => [new ClassSound(['sound' => 'LOOPFLAG'])]]),
    array_fill_keys($c_fountainOfFortune, ['sounds' => [new ClassSound(['sound' => 'LOOPFOUN'])]]),
    array_fill_keys($c_fountainOfYouth, ['sounds' => [new ClassSound(['sound' => 'LOOPFALL'])]]),
    array_fill_keys($c_gardenOfRevelation, ['sounds' => [new ClassSound(['sound' => 'LOOPGARD'])]]),
    array_fill_keys($c_subterraneanGate, ['sounds' => [new ClassSound(['sound' => 'LOOPGATE'])]]),
    array_fill_keys($c_stables, ['sounds' => [new ClassSound(['sound' => 'LOOPHORS'])]]),
    array_fill_keys($c_learningStone, ['sounds' => [new ClassSound(['sound' => 'LOOPLEAR'])]]),
    array_fill_keys($c_mysticalGarden, ['sounds' => [new ClassSound(['sound' => 'LOOPLEPR'])]]),
    array_fill_keys($c_shipyard, ['sounds' => [new ClassSound(['sound' => 'LOOPLUMB'])]]),
    array_fill_keys($c_schoolOfMagic, ['sounds' => [new ClassSound(['sound' => 'LOOPMAGI'])]]),
    array_fill_keys($c_tradingPost, ['sounds' => [new ClassSound(['sound' => 'LOOPMARK'])]]),
    array_fill_keys($c_mercenaryCamp, ['sounds' => [new ClassSound(['sound' => 'LOOPMERC'])]]),
    array_fill_keys($c_waterWheel, ['sounds' => [new ClassSound(['sound' => 'LOOPMILL'])]]),
    array_fill_keys($c_monolithOneWayEntrance, ['sounds' => [new ClassSound(['sound' => 'LOOPMON1'])]]),
    array_fill_keys($c_monolithOneWayExit, ['sounds' => [new ClassSound(['sound' => 'LOOPMON1'])]]),
    array_fill_keys($c_monolithTwoWay, ['sounds' => [new ClassSound(['sound' => 'LOOPMON2'])]]),
    array_fill_keys($c_temple, ['sounds' => [new ClassSound(['sound' => 'LOOPSANC'])]]),
    array_fill_keys($c_sanctuary, ['sounds' => [new ClassSound(['sound' => 'LOOPSANC'])]]),
    array_fill_keys($c_shrineOfMagicIncantation, ['sounds' => [new ClassSound(['sound' => 'LOOPSHRIN'])]]),
    array_fill_keys($c_shrineOfMagicGesture, ['sounds' => [new ClassSound(['sound' => 'LOOPSHRIN'])]]),
    array_fill_keys($c_shrineOfMagicThought, ['sounds' => [new ClassSound(['sound' => 'LOOPSHRIN'])]]),
    array_fill_keys($c_starAxis, ['sounds' => [new ClassSound(['sound' => 'LOOPSTAR'])]]),
    array_fill_keys($c_schoolOfWar, ['sounds' => [new ClassSound(['sound' => 'LOOPSWAR'])]]),
    array_fill_keys($c_tavern, ['sounds' => [new ClassSound(['sound' => 'LOOPTAV'])]]),
    array_fill_keys($c_whirlpool, ['sounds' => [new ClassSound(['sound' => 'LOOPWHIR'])]]),
    array_fill_keys($c_windmill, ['sounds' => [new ClassSound(['sound' => 'LOOPWIND'])]]),
    array_fill_keys($c_airElementalConflux, ['sounds' => [new ClassSound(['sound' => 'LOOPAIR'])]]),
    array_fill_keys($c_altarOfEarth, ['sounds' => [new ClassSound(['sound' => 'LOOPEART'])]]),
    array_fill_keys($c_altarOfThought, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_altarOfWater, ['sounds' => [new ClassSound(['sound' => 'LOOPFOUN'])]]),
    array_fill_keys($c_altarOfWishes, ['sounds' => [new ClassSound(['sound' => 'LOOPMAGI'])]]),
    array_fill_keys($c_archersTower, ['sounds' => [new ClassSound(['sound' => 'LOOPARCH'])]]),
    array_fill_keys($c_barracks, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_basiliskPit, ['sounds' => [new ClassSound(['sound' => 'LOOPMONS'])]]),
    array_fill_keys($c_behemothCrag, ['sounds' => [new ClassSound(['sound' => 'LOOPBEHE'])]]),
    array_fill_keys($c_blackMarket, ['sounds' => [new ClassSound(['sound' => 'LOOPMARK'])]]),
    array_fill_keys($c_boarGlen, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_buoy, ['sounds' => [new ClassSound(['sound' => 'LOOPBUOY'])]]),
    array_fill_keys($c_centaurStables, ['sounds' => [new ClassSound(['sound' => 'LOOPHORS'])]]),
    array_fill_keys($c_chapelOfStilledVoices, ['sounds' => [new ClassSound(['sound' => 'LOOPMEDU'])]]),
    array_fill_keys($c_cliffNest, ['sounds' => [new ClassSound(['sound' => 'LOOPBIRD'])]]),
    array_fill_keys($c_cloudTemple, ['sounds' => [new ClassSound(['sound' => 'LOOPTITA'])]]),
    array_fill_keys($c_crypt, ['sounds' => [new ClassSound(['sound' => 'LOOPDEAD'])]]),
    array_fill_keys($c_crystalCavern, ['sounds' => [new ClassSound(['sound' => 'LOOPCRYS'])]]),
    array_fill_keys($c_cursedGround, ['sounds' => [new ClassSound(['sound' => 'LOOPCURS'])]]),
    array_fill_keys($c_cursedTemple, ['sounds' => [new ClassSound(['sound' => 'LOOPSKEL'])]]),
    array_fill_keys($c_cyclopsCave, ['sounds' => [new ClassSound(['sound' => 'LOOPCAVE'])]]),
    array_fill_keys($c_demonGate, ['sounds' => [new ClassSound(['sound' => 'LOOPCAVE'])]]),
    array_fill_keys($c_dendroidArches, ['sounds' => [new ClassSound(['sound' => 'LOOPGARD'])]]),
    array_fill_keys($c_dragonCave, ['sounds' => [new ClassSound(['sound' => 'LOOPDRAG'])]]),
    array_fill_keys($c_dragonCliffs, ['sounds' => [new ClassSound(['sound' => 'LOOPDRAG'])]]),
    array_fill_keys($c_dragonUtopia, ['sounds' => [new ClassSound(['sound' => 'LOOPDRAG'])]]),
    array_fill_keys($c_dragonVault, ['sounds' => [new ClassSound(['sound' => 'LOOPDRAG'])]]),
    array_fill_keys($c_dwarfCottage, ['sounds' => [new ClassSound(['sound' => 'LOOPDWAR'])]]),
    array_fill_keys($c_earthElementalConflux, ['sounds' => [new ClassSound(['sound' => 'LOOPEART'])]]),
    array_fill_keys($c_elementalConflux, ['sounds' => [new ClassSound(['sound' => 'LOOPTITA'])]]),
    array_fill_keys($c_enchantedSpring, ['sounds' => [new ClassSound(['sound' => 'LOOPPEGA'])]]),
    array_fill_keys($c_enchanterHollow, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_estate, ['sounds' => [new ClassSound(['sound' => 'LOOPDEAD'])]]),
    array_fill_keys($c_fireElementalConflux, ['sounds' => [new ClassSound(['sound' => 'LOOPFIRE'])]]),
    array_fill_keys($c_fireLake, ['sounds' => [new ClassSound(['sound' => 'LOOPVENT'])]]),
    array_fill_keys($c_forsakenPalace, ['sounds' => [new ClassSound(['sound' => 'LOOPDEVL'])]]),
    array_fill_keys($c_gnollHut, ['sounds' => [new ClassSound(['sound' => 'LOOPORC'])]]),
    array_fill_keys($c_goblinBarracks, ['sounds' => [new ClassSound(['sound' => 'LOOPGOBL'])]]),
    array_fill_keys($c_goldenPavilion, ['sounds' => [new ClassSound(['sound' => 'LOOPNAGA'])]]),
    array_fill_keys($c_golemFactory, ['sounds' => [new ClassSound(['sound' => 'LOOPFACT'])]]),
    array_fill_keys($c_gorgonLair, ['sounds' => [new ClassSound(['sound' => 'LOOPBEHE'])]]),
    array_fill_keys($c_graveyard, ['sounds' => [new ClassSound(['sound' => 'LOOPDEAD'])]]),
    array_fill_keys($c_griffinTower, ['sounds' => [new ClassSound(['sound' => 'LOOPGRIF'])]]),
    array_fill_keys($c_guardhouse, ['sounds' => [new ClassSound(['sound' => 'LOOPPIKE'])]]),
    array_fill_keys($c_hallOfDarkness, ['sounds' => [new ClassSound(['sound' => 'LOOPDEAD'])]]),
    array_fill_keys($c_hallOfSins, ['sounds' => [new ClassSound(['sound' => 'LOOPVENT'])]]),
    array_fill_keys($c_harpyLoft, ['sounds' => [new ClassSound(['sound' => 'LOOPHARP'])]]),
    array_fill_keys($c_hellHole, ['sounds' => [new ClassSound(['sound' => 'LOOPFIRE'])]]),
    array_fill_keys($c_hillFort, ['sounds' => [new ClassSound(['sound' => 'LOOPSWAR'])]]),
    array_fill_keys($c_homestead, ['sounds' => [new ClassSound(['sound' => 'LOOPELF'])]]),
    array_fill_keys($c_hydraPond, ['sounds' => [new ClassSound(['sound' => 'LOOPHYDR'])]]),
    array_fill_keys($c_impCrucible, ['sounds' => [new ClassSound(['sound' => 'LOOPFIRE'])]]),
    array_fill_keys($c_kennels, ['sounds' => [new ClassSound(['sound' => 'LOOPDOG'])]]),
    array_fill_keys($c_labyrinth, ['sounds' => [new ClassSound(['sound' => 'LOOPANIM'])]]),
    array_fill_keys($c_lizardDen, ['sounds' => [new ClassSound(['sound' => 'LOOPARCH'])]]),
    array_fill_keys($c_mageTower, ['sounds' => [new ClassSound(['sound' => 'LOOPMAGI'])]]),
    array_fill_keys($c_magicForest, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_magicLantern, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_magicPlains, ['sounds' => [new ClassSound(['sound' => 'LOOPMAGI'])]]),
    array_fill_keys($c_manticoreLair, ['sounds' => [new ClassSound(['sound' => 'LOOPMANT'])]]),
    array_fill_keys($c_mausoleum, ['sounds' => [new ClassSound(['sound' => 'LOOPDEAD'])]]),
    array_fill_keys($c_monastery, ['sounds' => [new ClassSound(['sound' => 'LOOPMONK'])]]),
    array_fill_keys($c_nomadTent, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_ogreFort, ['sounds' => [new ClassSound(['sound' => 'LOOPOGRE'])]]),
    array_fill_keys($c_orcTower, ['sounds' => [new ClassSound(['sound' => 'LOOPORC'])]]),
    array_fill_keys($c_parapet, ['sounds' => [new ClassSound(['sound' => 'LOOPGRIF'])]]),
    array_fill_keys($c_pillarOfEyes, ['sounds' => [new ClassSound(['sound' => 'LOOPCAVE'])]]),
    array_fill_keys($c_pillarOfFire, ['sounds' => [new ClassSound(['sound' => 'LOOPFIRE'])]]),
    array_fill_keys($c_portalOfGlory, ['sounds' => [new ClassSound(['sound' => 'LOOPSANC'])]]),
    array_fill_keys($c_pyre, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_refugeeCamp, ['sounds' => [new ClassSound(['sound' => 'LOOPMERC'])]]),
    array_fill_keys($c_rogueCavern, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_serpentFlyHive, ['sounds' => [new ClassSound(['sound' => 'LOOPLEAR'])]]),
    array_fill_keys($c_sirens, ['sounds' => [new ClassSound(['sound' => 'LOOPSIRE'])]]),
    array_fill_keys($c_sulfurousLair, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_thatchedHut, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_tombOfCurses, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_tombOfSouls, ['sounds' => [new ClassSound(['sound' => 'LOOPDEAD'])]]),
    array_fill_keys($c_trainingGrounds, ['sounds' => [new ClassSound(['sound' => 'LOOPHORS'])]]),
    array_fill_keys($c_treetopTower, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_trollBridge, ['sounds' => [new ClassSound(['sound' => 'LOOPSWOR'])]]),
    array_fill_keys($c_unicornGlade, ['sounds' => [new ClassSound(['sound' => 'LOOPUNIC'])]]),
    array_fill_keys($c_volcano, ['sounds' => [new ClassSound(['sound' => 'LOOPVOLC'])]]),
    array_fill_keys($c_warren, ['sounds' => [new ClassSound(['sound' => 'LOOPCAVE'])]]),
    array_fill_keys($c_waterElementalConflux, ['sounds' => [new ClassSound(['sound' => 'LOOPFOUN'])]]),
    array_fill_keys($c_wolfPen, ['sounds' => [new ClassSound(['sound' => 'LOOPWOLF'])]]),
    array_fill_keys($c_workshop, ['sounds' => [new ClassSound(['sound' => 'LOOPGREM'])]]),
    array_fill_keys($c_wyvernNest, ['sounds' => [new ClassSound(['sound' => 'LOOPMONS'])]]),
  ]),
];